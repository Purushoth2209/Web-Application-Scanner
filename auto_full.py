    ...
    def _crawl(url, d):
        ...
        try:
            ...
            for f in form_elems:
                ...
                params = {
                    i.get_attribute("name") or f"field_{idx}": i.get_attribute("value") or ""
            ...
            # also include hash routes (Angular/React)
            hrefs = driver.execute_script(
                "return Array.from(document.querySelectorAll('[href]')).map(a => a.getAttribute('href'));"
            )
            for h in hrefs:
...
if __name__ == "__main__":
    ...
    cookies, jwt = get_tokens(
        args.base, args.login_path, args.username,
